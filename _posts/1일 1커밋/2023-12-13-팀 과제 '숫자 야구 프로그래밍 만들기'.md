...
## 1. 컴퓨터가 0과 9사이의 서로 다른 숫자 3개를 무작위로 생성
   ...
   ```
   > set 을 사용하기 위해서는 위와 같이 ``java.util.HashSet``와 ``java.util.Set``를 추가해줘야 한다 ( ``import java.util.*;``으로 정리함)
   ...
   **문제점**
   ...
   - 트러블 슈팅: list를 생성하고 ``Collections.shuffle(list);``로 ``.shuffle``를 넣어주니 리스트가 랜덤 정렬이 되었다. 
